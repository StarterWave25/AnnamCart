...
async function checkStatus(mobile) {
    const request = await fetch(`http://localhost/AnnamCart/tirumala/delivery-partner/data/data-status.php?dmobile=${mobile}`);
    const response = await request.json();
    if (response.status == 'active') {
        await fetch('http://localhost/AnnamCart/tirumala/delivery-partner/data/data-dactivity.php?mode=inactive');
...
async function setRestaurantStatus(status, rid) {
    const request = await fetch(`http://localhost/AnnamCart/tirumala/restaurant-admin/data/data-status.php?rid=${rid}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
