...
async function getRestaurantData(restaurantId) {
  ...
  const cartedItems = getQuantityStorage();
  let cartItemHTML = '';
  let restaurantItemsHTML = '';

  restaurantData.resBody.forEach(async (item) => {
    let flag = false;
    cartedItems.forEach((cartItem) => {
    ...
    popularItems.forEach((p_item) => {
      if (p_item == item.item_id) {
        document.querySelector(`.js-food-card-${item.item_id}`).id = 'popular-item';
      }
    });
    getAddBtnHTML(Number(item.item_id));
  });

  resBody.innerHTML = cartItemHTML + restaurantItemsHTML;
...
if (targetId) {
  // 2. Wait for the dynamic content to load and the element to be available
  const checkAndScroll = () => {
    const targetElement = document.getElementById(targetId);

    if (targetElement) {
      // 3. Scroll to the target element once it's found
